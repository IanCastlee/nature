...
if ($method === "GET") {
    ...
    $stmt = $conn->prepare("SELECT * FROM inclusions WHERE room_id = ?");
...
if ($method === 'POST') {
    ...
    if ($action === "create") {
        ...
        $checkStmt = $conn->prepare("SELECT inclusion FROM inclusions WHERE room_id = ? AND inclusion = ?");
        ...
        if ($result->num_rows < 1) {
            // Insert new amenity
            $insertStmt = $conn->prepare("INSERT INTO inclusions (room_id, inclusion) VALUES (?, ?)");
    ...
    if ($action === "update") {
        ...
        $stmt = $conn->prepare("UPDATE inclusions SET room_id = ?, inclusion = ? WHERE inclusion_id = ?");
    ...
    if ($action === "delete") {
      ...
      $stmt = $conn->prepare("DELETE FROM inclusions WHERE inclusion_id = ?");

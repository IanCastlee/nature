...
function CustomDropDownn({ options, label, value, onChange }) {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef(null);

  const selectedOption = options?.find(
    (option) => option.category_id === value
  );

  const toggleDropdown = () => setIsOpen(!isOpen);

  const handleOptionClick = (option) => {
    onChange(option.category_id);
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <label htmlFor="custom-dropdown" className="text-xs dark:text-gray-300">
        {label}
      </label>
      <button
        type="button"
      ...
      >
        {selectedOption ? (
          ...
          <span className="text-gray-400">-- Select a title --</span>
        )}
